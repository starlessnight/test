...
public class Create_activity extends Activity{
	...
	private String initEndDateTime ,hr2,id="0"; // 初始化结束时间  
	...
	private void AddJob(){
		...
		if(select_witch!=1){
         ...
         now_time = spliteString(timeShow.getText().toString(), "日", "index", "back"); // 时间  
			...
			hr2= spliteString(initEndDateTime, "日", "index", "back");
			hr=Integer.parseInt(hr2.substring(1, hr2.indexOf(":")));
	        min=Integer.parseInt(spliteString(hr2, ":", "index", "back"))+Integer.parseInt(now_time);
	        if(min>=60){
	        	min=min%60;
	        	hr=hr+(min/60);
	        }
		...
		if(check>0){
			...
			if(select_witch!=1){
				...
				now_time=reciprocal.getText().toString();
					hr2= spliteString(initEndDateTime, "日", "index", "back");
					System.out.println("倒數1"+ hr2);
					year=Integer.parseInt(spliteString(now_date, "年", "index", "front"));
			        month= Integer.parseInt(now_date.substring(5,7));    
			        day= Integer.parseInt(now_date.substring(8,10));
			        hr=Integer.parseInt(hr2.substring(1, hr2.indexOf(":")));
		            ...
		            c.set(Calendar.SECOND,0);
		              ...
		              send.edit()
		              .putString("Alarm", String.valueOf(Integer.valueOf(id)+1))
		              .commit();
